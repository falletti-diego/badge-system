...
describe('POST /api/v1/checkins — event conflict guard', () => {
  ...
  afterAll(async () => {
    // Nota: closePool() sul modulo condiviso db/pool NON va chiamato qui — questo
    // file ha un secondo describe (PUT) più sotto che richiede lo stesso `app` e
    // quindi lo stesso pool condiviso; chiuderlo due volte fa fallire la seconda
    // suite. Viene chiuso una sola volta, nell'afterAll dell'ultimo describe.
