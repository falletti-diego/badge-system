...
describe('applyDiff', () => {
  ...
  it('includes manager_id when inserting a new employee', async () => {
    const db = mockClient([['INSERT INTO employees', { rows: [{ id: 'emp-new' }] }]]);
    const diff = { nuovi: [{ email: 'nuovo@x.it', name: 'Nuovo', role: 'employee', site_id: 's1', hiring_date: '2026-07-01', manager_id: 'mgr-1' }], riattivati: [], rimossi: [], modificati: [] };
    await applyDiff(db, diff, { clientId: 'client-1' });
    const call = db.query.mock.calls.find((c) => c[0].includes('INSERT INTO employees'));
    expect(call).toBeDefined();
    expect(call[0]).toMatch(/manager_id/);
    expect(call[1]).toContain('mgr-1');
  });
