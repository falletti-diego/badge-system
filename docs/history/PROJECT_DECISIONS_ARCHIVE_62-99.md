# Badge System — PROJECT_DECISIONS Archivio (Sessioni 62-99 + Documento di Architettura Originale)

> **Archiviato il 2026-09-12** per ridurre la dimensione di `PROJECT_DECISIONS.md` (era 2970 righe). Contenuto spostato qui verbatim, nessuna informazione persa — solo le sessioni più recenti (100+) restano in `PROJECT_DECISIONS.md`.
>
> Questo file contiene: (1) il log decisionale delle Sessioni 62-99, (2) in coda, il documento di architettura/planning originale del progetto (Sezioni 1-9: Project Overview, Tech Stack, Fasi di Sviluppo, Decision Points, Risk Register, ecc. — largamente sovrapposto a `CLAUDE.md` e ai reference `01_BRAINSTORMING_SISTEMA.md`/`02_ARCHITETTURA_APPROVATA.md`).
...
### Session 113: Checkout locale disallineato da `origin/main` (5 commit) + recupero doc mai committati (24 Agosto 2026)

**Contesto:** l'utente ha chiesto conferma sullo stato del bug "durata/giorno evento in Presenze" (Session 106), che riteneva già risolto. Verifica del `git log` locale mostrava il bug ancora come aperto — ma il `main` locale risultava **5 commit indietro rispetto a `origin/main`** (mancavano: PR #11 mutua esclusione Smart Working↔Eventi, PR #12 fix CI Node 20/actions v5, pacchetto "Sales-Ready" con mitigazione S.27/S.28/S.29, chiusura Task 10 DNS Route53, handoff Session 111-112). Il bug era in realtà già chiuso da tempo (Session 110), solo invisibile dal checkout locale stale.

**Decisione:** `git pull --ff-only origin main` — fast-forward pulito, nessun conflitto, nessuna modifica di contenuto esistente. Verificato working tree pulito prima del pull (solo file non tracciati, non toccati dall'operazione).

**Secondo problema trovato durante la verifica del `git status` post-sync:** 5 file rimasti solo in locale, mai committati in nessuna sessione precedente:
- 3 piani di implementazione (`docs/superpowers/plans/2026-06-21-mobile-leave-illness-step1.md`, `2026-06-25-mobile-bug-fix-qr-ferie.md`, `2026-08-20-eventi-training.md`) — verificato via `git log --follow` che non erano mai stati nella storia sotto nessun nome. Le feature che descrivono sono tutte già implementate e live in produzione: il gap era solo documentale, non di codice.
- 1 checklist di verifica manuale staging (`docs/verifica-staging-fase-a-2026-08-05.md`) — tutte le caselle vuote, verifica già effettuata verbalmente in Session 93 (mergiata con successo in `main` quella stessa sessione).
- 1 build APK locale (186MB) — mai adatta a un commit git, rigenerabile in qualsiasi momento.

**Azione:** committati i 4 file `.md` così come trovati (nessuna riscrittura di contenuto), aggiunto `*.apk` a `.gitignore`, rimossa la build binaria dal disco. Commit `442a40a`, push su `origin/main`.

**Lezione di processo:** questa è la stessa classe di problema già documentata in Session 41 e Session 89 ("N commit mai pushati") — ma qui la causa non era una sessione che dimenticava di pushare, bensì un checkout locale che non veniva più aggiornato da `origin` mentre altre sessioni (probabilmente da worktree isolate, come già visto in Session 111-112) pushavano direttamente. **Prima di fidarsi di `git log`/`TASKS.md`/`HANDOFF.md` locali per rispondere su "cosa è stato fatto"**, verificare `git fetch` + confronto `main...origin/main` quando la risposta attesa dall'utente non coincide con quella osservata — un segnale che i due erano disallineati, non che la memoria del progetto fosse sbagliata.

---

### Session 114: Precisazione claim marketing (Face ID) + design/piano mutua esclusione Evento/Ferie/Malattia (25 Agosto 2026)

**Parte A — Marketing (nessun codice applicativo toccato):** Aggiunta la pianificazione turni come differenziatore in `.agents/product-marketing.md` (v5, prima assente dal documento nonostante feature live da tempo). Poi, su domanda diretta dell'utente sul funzionamento reale del check-in, verifica nel codice (`checkins.js`, `QRScannerScreen.jsx`, `FaceIDScreen.jsx`) ha smentito il claim "Face ID lega il check-in fisicamente alla persona": Face ID è opzionale (disattivabile in Impostazioni), verifica solo lo sblocco biometrico/PIN **del dispositivo** (non un confronto col volto specifico dell'account), e `faceid_verified` è salvato come metadato mai usato come gate server-side. Corretto in `.agents/product-marketing.md` (v6) e nei due asset di outreach già scritti (`docs/marketing/cold-email-outreach-template.md`, `one-pager-badge-system.md`) — **prima** di iniziare l'outreach reale, per evitare un'obiezione di credibilità con un prospect tecnico in demo. Proposto (non eseguito) un piano per validare il gap di prezzo 1,3-1,9x vs NoBadge tramite le prime 10-15 conversazioni di cold outreach.

**Parte B — Bug reale in produzione, causa radice + design + piano (nessuna implementazione eseguita):** L'utente ha segnalato e fatto verificare un bug riprodotto manualmente: `maria@badge.local`, 25/08/2026, con Evento approvato + Ferie approvata + Malattia comunicata tutti attivi sullo stesso giorno — dovrebbero essere mutuamente esclusivi a coppie. Causa radice diagnosticata leggendo il codice reale (non ipotizzata): `backend/src/utils/eventConflict.js` esiste già dalla feature Eventi/Training, ma applicato in modo asimmetrico — `events.js` controlla ferie/malattia solo in creazione, non in approvazione; `leaves.js` e `illnesses.js` non controllano nulla in nessun punto.

**Processo:** `/superpowers:brainstorming` con domande one-at-a-time (punto di blocco creazione+approvazione, "malattia vince sempre" con auto-cancellazione, Ferie/Evento bloccati a vicenda in invio, retroattività limitata a oggi/futuro) → **due round di analisi critica esplicitamente richiesti dall'utente** prima di scrivere la spec, non uno solo. Il primo round ha trovato 2 rischi critici concreti verificati nel codice: **(1)** `leave_saldi.used_days` viene incrementato in approvazione ma non esiste in nessun punto del backend un percorso che lo decrementi — un'auto-cancellazione da malattia su una ferie già approvata avrebbe fatto perdere permanentemente quei giorni al dipendente se non corretta; **(2)** l'auto-cancellazione su una data passata avrebbe potuto alterare silenziosamente ore/buoni pasto già esportati verso il commercialista (Zucchetti/TeamSystem) — risolto limitando la cascata a date odierne/future. Il secondo round, su richiesta esplicita di valutare "soluzioni allo stato dell'arte", ha trovato un **quarto punto di scrittura non protetto da nessuno dei controlli progettati**: `backend/src/utils/demoSeed.js` (il tenant demo self-service, lo stesso asset usato per l'outreach commerciale) inserisce `leave_requests`/`illnesses` con SQL diretto, bypassando ogni route — non un bug attivo oggi (un offset hard-coded evita la sovrapposizione per convenzione, non per garanzia), ma strutturalmente fragile. Valutate e scartate esplicitamente due alternative allo stato dell'arte (Postgres `EXCLUDE` constraint — non esprime esclusione cross-tabella e non supporta la cascata; trigger DB cross-tabella — stesso pattern già usato con successo per l'invariante `assigned_sites` in Session 93, ma sposterebbe audit-log e cascata in PL/pgSQL rompendo la coerenza con `logAudit()`, rimandato esplicitamente come hardening futuro non bloccante) in favore di un fix mirato: `demoSeed.js` chiama la stessa funzione condivisa di conflict-check invece di affidarsi all'offset implicito, più un nuovo "Known Bug Pattern 7" da aggiungere a `CLAUDE.md` durante l'implementazione.

**Deliverable:** design spec (`docs/superpowers/specs/2026-08-25-event-leave-illness-mutual-exclusion-design.md`, con entrambi i round di critica integrati) e piano di implementazione TDD via `/superpowers:writing-plans` (`docs/superpowers/plans/2026-08-25-event-leave-illness-mutual-exclusion.md`, 8 task, ogni step con test eseguibile e codice reale, nessun placeholder — verificato riga per riga contro il codice sorgente attuale, incluse le firme di funzione, gli schemi Zod, e i pattern di test esistenti nel repo prima di scrivere il piano).

**Non fatto in questa sessione:** nessun codice applicativo del piano è stato eseguito (solo documentazione/marketing). **Cleanup del dato corrotto di Maria in produzione: rimandato esplicitamente** — richiederebbe SSH sull'EC2 di produzione, bloccato dal classificatore di sicurezza automatico di questa sessione; l'utente ha scelto di rimandarlo invece di autorizzarlo esplicitamente.

**Lezione di processo:** un'analisi critica non è "fatta una volta" — la seconda passata, esplicitamente richiesta dall'utente con "valuta soluzioni allo stato dell'arte", ha trovato un gap reale (`demoSeed.js`) che la prima passata (concentrata sulle 3 route HTTP) non aveva considerato. Lo stesso pattern già osservato in Session 93 ("una review per-task non sostituisce una passata finale sull'insieme") si applica anche a un singolo design: la domanda "quali altri punti scrivono su queste tabelle?" è un `grep` che vale la pena fare sempre, non solo quando il sospetto emerge da solo.

---

**Last Updated:** 25 Agosto 2026 (Session 114 — precisazione claim Face ID in marketing + design/piano mutua esclusione Evento/Ferie/Malattia, implementazione non ancora avviata)
