...
- [ ] **Step 4: creare l'AVD di fascia bassa** — stesso wizard. **Verificato (2026-07-27): le immagini "Go edition" non sono distribuite tramite l'SDK Manager standard di Android Studio** (richiesta a Google dal 2019, mai spedita nel canale di download normale) — non cercarle, non sono lì. Percorso corretto: scegliere un device definition semplice (es. **Nexus S**, schermo piccolo) con il system image **API level più basso disponibile** tra quelli proposti, che sia comunque supportato dalla build (vedi Task 2, Step 3 per come verificare il `minSdkVersion` reale). Nella schermata "Verify Configuration", cliccare **"Show Advanced Settings"** e impostare **RAM a 1536 MB** in "Memory and Storage" — questo, non l'etichetta "Go", è ciò che approssima davvero un device economico. Nome AVD: `Android_Go_LowSpec` (il nome resta invariato per coerenza col resto del piano, anche se non è un'immagine Go autentica).
