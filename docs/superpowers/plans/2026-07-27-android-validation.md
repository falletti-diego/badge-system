...
- [ ] **Step 4: creare l'AVD di fascia bassa ("Go edition")** — stesso wizard, ma scegliere un system image **"Android Go"** (nella lista system image, cercare la voce con etichetta "Go" — tipicamente disponibile per API 28-30, pensata esplicitamente per device con RAM limitata, l'esatto profilo hardware di un telefono economico). Se nessuna immagine Go è disponibile per il proprio Android Studio, usare in alternativa un device definition **Nexus S** (schermo piccolo, specifica bassa) con l'API level più basso disponibile che sia comunque supportato dalla build (vedi Task 2, Step 4 per come verificarlo). Nome AVD: `Android_Go_LowSpec`.
