...
- CSS di stampa iniettato con `<GlobalStyles>` di MUI, scoped alla pagina:
  ...
  - Mostra un `.print-title` (altrimenti `display: none`) con testo `📊 Riepilogo Ore — {nomeMese} {anno}` — stesso formato/stile di Planning (`📅 Planning Turni — {mese} {anno}`, verificato in `PlanningPage.jsx`), visibile solo in stampa
...
- Filtra `FAQ_ITEMS` con `isVisible(item, role)` (vedi sopra), `role` letto con `authService.getUserRole()` — **esplicitamente questo e non `useAuth()`**: il codebase ha già due meccanismi coesistenti (`useAuth()` in `NavBar.jsx`, `authService.getUserRole()` diretto in `SummaryPage.jsx`), e `HelpPage` è nello stesso gruppo di navigazione/scopo di `SummaryPage` — usa lo stesso meccanismo di quella, non quello di `NavBar`, per coerenza con la pagina a cui è più affine.
- `getUserRole()` è **sincrono** (legge da `localStorage` già popolato al momento del render, nessuna chiamata asincrona) — a differenza del mobile, qui non esiste una vera race di "ruolo non ancora caricato": il filtro fail-closed resta comunque la difesa corretta (nessun costo aggiuntivo, copre anche il caso limite di un utente il cui `localStorage` non contenga alcun ruolo), ma non serve alcuno stato di loading/spinner sulla pagina web.
- **Rischio pre-esistente noto, non introdotto da questo piano**: `authService.getUser()` fa `JSON.parse(localStorage.getItem(...))` senza try/catch — un valore corrotto in `localStorage` lo farebbe fallire in modo non gestito. `HelpPage` diventa un nuovo chiamante esposto allo stesso rischio già presente oggi in `SummaryPage`/`NavBar`. Non è nello scope di questo piano correggere `authService.js` (fuori dai file toccati, nessuna feature qui lo richiede) — annotato qui per tracciabilità, eventuale fix va proposto come voce di backlog separata.
...
- A differenza del web, qui `getUser()` è **realmente asincrono** — nel breve intervallo prima che risolva, lo schermo mostra solo le FAQ `audience: 'all'` (comportamento accettabile: un lieve "flash" di contenuto ridotto per una manciata di millisecondi, non un errore, non richiede uno spinner dedicato — coerente con quanto già accettato per la lettura del ruolo in `MainTabs`).
...
I due file dati sono mantenuti manualmente in sync (nessun package condiviso, per scelta esplicita — vedi sopra). Questo è un rischio reale di drift silenzioso (qualcuno aggiorna una FAQ solo lato web, mobile resta indietro). Mitigazione: **script di verifica** `scripts/check-faq-sync.js` alla radice del repo. **Vincolo tecnico verificato**: `frontend-web/package.json` dichiara `"type": "module"` (ESM puro, `export const`), mentre uno script Node in `scripts/` gira in CommonJS di default — un `require()`/`import()` diretto dei due `faq.js` come moduli reali fallirebbe o richiederebbe di allineare i sistemi di modulo tra progetti diversi (rischio non necessario). Lo script quindi **non esegue i file**: legge entrambi come testo grezzo (`fs.readFileSync`) ed estrae le triple `id`/`question`/`answer` con una regex sull'oggetto letterale (formato controllato, generato da questa stessa spec — la regex non deve gestire JS arbitrario, solo il pattern fisso `{ id: '...', question: '...', answer: '...', audience: '...' }` ripetuto). Confronta i due insiemi di triple, fallisce con exit code ≠0 su qualunque divergenza (id mancante da un lato, testo diverso, `audience` diverso). `.github/workflows/ci.yml` oggi ha 3 job (`backend`, `mobile`, `security-check`), nessuno dei quali è il posto naturale per un controllo che non appartiene né al backend né al mobile — va aggiunto come step extra al job `backend` (che ha già Node 20 configurato, il setup più leggero disponibile) piuttosto che creare un quarto job solo per questo. Così un PR che tocca un solo `faq.js` senza l'altro fallisce la CI invece di scoprirsi in produzione.
