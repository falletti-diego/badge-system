...
**Last Updated:** 15 Luglio 2026 (Session 66 — Task 6/9 demo self-service (`DEMO_EXPIRED` su refresh + cleanup scheduler) implementato e chiuso, bypass reale trovato dal code-review e fixato)  
**Status:** Deploy produzione ✅ LIVE (badge.dataxiom.it) | Mobile Build 26 ✅ (vibrazione check-in) | Grafici Trend Dashboard ✅ LIVE (api.dataxiom.it) | Dropdown Sede in Dashboard ✅ LIVE | Export CSV date/ora ✅ fix live | Pipeline CI/CD ✅ funzionante (backend + mobile) | ✅ Hotfix `POST /auth/refresh` MERGIATO su main e verificato live in produzione (login → refresh reale confermato 200, replay correttamente rifiutato 401) | 🚧 Ambiente Demo Self-Service — Task 3/9-6/9 ✅ chiusi, Task 7/9 (`TryDemoPage.jsx`) da iniziare (worktree isolato, non ancora in main)  
**MVP Launch Target:** Settembre 2026 | **Current Phase:** Ambiente Demo Self-Service da Task 7/9 (piano approvato, esecuzione a task singoli con pausa esplicita), poi MVP Hardening backlog residuo (Session 57) o staging environment + ONB.2
