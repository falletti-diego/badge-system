...
**Last Updated:** 2026-07-30 (Session 89: SES fuori sandbox — AWS ha approvato il sandbox-exit, Task 7 del piano SES chiuso [MAX_ACTIVE_DEMOS in produzione, test E2E reale verso indirizzo non verificato], Gate finale onboarding prossimo passo)  
...
|---|---|---|
| 89 | 2026-07-30 | **✅ SES fuori sandbox — AWS ha approvato la richiesta (ferma dal `DENIED` di Session 84), Task 7 del piano SES chiuso** — L'utente ha ricevuto la comunicazione AWS di approvazione (quota 50.000 email/giorno, 14/sec) senza che fosse stato necessario alcun canale alternativo tra quelli valutati in Session 85/86 (nessuna azione presa, la risposta è arrivata da sola). Verificato via CLI: `ProductionAccessEnabled: true`, `ReviewStatus: GRANTED`. Eseguito **Task 7** del piano SES (`docs/superpowers/plans/2026-07-19-demo-funnel-screenshots-ses.md`, ultimo passo mai chiuso): `aws ssm put-parameter /badge/production/MAX_ACTIVE_DEMOS=20` (mai impostato prima, verificato), `docker restart badge-system-api` su EC2 (validazione 18/18 variabili confermata nei log di boot), test E2E reale in produzione — email inviata con successo (`MessageId` valido) verso un indirizzo mai verificato (prova diretta e concreta dell'uscita dal sandbox, non solo verso `diego@dataxiom.it` come nel fix parziale di Session 75), più un secondo test attraverso il flusso reale pubblico (`POST /demo/start` → `POST /demo/contact`, mirror esatto del form "Parliamo"). Entrambi riusciti. Tenant demo di test ripulito dal DB di produzione dopo la verifica (stessa convenzione di cleanup già usata nelle sessioni precedenti). **Sblocca**: il Gate finale del piano onboarding self-service (Session 85, 8/8 task già implementati — resta solo questa verifica E2E con email reali) e l'invio verso prospect reali sul funnel demo pubblico. Su scelta esplicita dell'utente, il Gate finale onboarding è il prossimo passo di questa stessa sessione. |
...
| **[x] Onboarding cliente self-service** (wizard nel dashboard) | Alto (scala) | Alto | ✅ Completato Session 85 — invito admin via email + token one-time, `/admin/onboarding` wizard (upload Excel → preview/diff → apply), welcome email dipendenti automatica. Riusa ancora il template Excel di `ONB.1`. Gate finale (verifica E2E con SES reale) resta bloccato dal sandbox-exit AWS |
