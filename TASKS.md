...
| **[ ] Bug UX: redirect post-login per admin con onboarding incompleto** | Alto (primo impatto cliente reale) | Basso | Scoperto Session 89 durante Gate finale: `LoginPage.jsx:44` naviga sempre a `/dashboard` dopo il login, incondizionatamente. Un admin appena creato via invito (nessuna sede/dipendente ancora caricata dal wizard) atterra su una dashboard/planning completamente vuoti, senza alcun link per tornare a `/admin/onboarding` — vicolo cieco per il primo utente reale di un nuovo cliente. Fix: il login (o l'oggetto `user`) deve esporre se il client ha già ≥1 sede, e `LoginPage` deve reindirizzare ad `/admin/onboarding` invece che `/dashboard` in quel caso |
