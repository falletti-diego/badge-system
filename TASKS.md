...
**Last Updated:** 2026-07-30 (Session 87: ANDROID.2 completato lato codice — jank animazioni Android low-end; verifica numerica su device bloccata da un problema d'ambiente pre-esistente, non dal fix)  
...
- [x] **ANDROID.2** ✅ **Codice completato (Session 86, 2026-07-29/30)** — piano `docs/superpowers/plans/2026-07-29-android-jank-lowend.md` (spec: `docs/superpowers/specs/2026-07-29-android-jank-lowend-design.md`). Scoperta chiave: le animazioni già usavano `useNativeDriver: true` ovunque (verificato via Explore) — l'ipotesi originaria era già soddisfatta, il jank derivava dal costo di compositing GPU complessivo (specialmente la `CameraView` live in `QRScannerScreen`), non dalla complessità delle animazioni. Fix implementato: nuovo `frontend-mobile/src/utils/deviceTier.js` (`isLowEndDevice()`, soglia RAM ≤3GB via `expo-device`) consumato da `FaceIDScreen.jsx`/`QRScannerScreen.jsx` per disattivare, solo sui device rilevati come low-end, le due animazioni puramente decorative (arco rotante, pallino di stato) — scan-line e ring-pulse (funzionali) restano sempre attivi su tutti i device. TDD completo: 13 nuovi test (5 `deviceTier`, 4 `FaceIDScreen`, 4 `QRScannerScreen`), red→green verificato per ognuno; suite mobile completa 75/75 verde, nessuna regressione.
  - [ ] **Verifica numerica su device rimasta bloccata da un problema d'ambiente, non dal codice**: build EAS locale riuscita (`eas build --local`), ma sia questa nuova APK sia una build precedente della Session 83 (installata per isolare la causa) falliscono identicamente all'avvio su **entrambi** gli AVD (`Android_Go_LowSpec` E `Pixel_6_API_34`, quest'ultimo appena riavviato) con `am start` → `Error: Activity class ... does not exist`, nonostante manifest/resolver table/`dumpsys package` confermino tutti che l'activity è dichiarata correttamente (`exported=true`, intent-filter corretto) — verificato anche dopo reinstallazione pulita e reboot completo dell'emulatore. Il fatto che una build immutata della Session 83 fallisca allo stesso modo conferma che non è una regressione di questo fix, ma un problema dell'ambiente Android locale (toolchain `adb`/emulator) insorto dopo quella sessione. **Da riprendere**: diagnosticare l'ambiente locale (verificare versioni `adb`/`platform-tools`, considerare di ricreare gli AVD da zero) prima di poter ri-eseguire `dumpsys gfxinfo` e confrontare i numeri con la baseline (100%/99,77%).
