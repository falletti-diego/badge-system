...
| 2026-06-11 | GDPR Blockers Implementation (Session 33) | S.24 ✅, S.25 ✅, S.26 ✅ | **S.24 — GPS Data Disclosure (Art. 13-14):** docs/privacy-policy-IT.md (Italian Privacy Policy + GPS section, 90-day retention, Art. 6(1)(b) or (f) base legal, AWS sub-processor), GPSConsentDialog mobile component, QRScannerScreen consent flow. **S.25 — DPA Template (Art. 28):** docs/DPA_GDPR_Art28_IT.md (full template, client signature required), migration 011 (dpa_acknowledgements), backend POST/GET /api/admin/dpa-acknowledgement. **S.26 — Explicit Consent (Art. 7):** migration 012 (employees.gps_consent_given, employee_consent_log), backend POST /api/consent/gps-acceptance + GET /api/consent/my-consents, mobile consent dialog + AsyncStorage + audit trail. All 216 tests PASS ✅. Commit: b6684ac. **Deferred phase 2:** AdminPage "Consensi GPS" tab (monitoring only, not compliance-critical) |
