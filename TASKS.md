...
**Last Updated:** 2026-07-29 (Session 85: onboarding cliente self-service implementato — 8/8 task del piano + code review finale, 5 fix reali trovati e risolti; SES ancora `DENIED`, nessuna risposta AWS)  
...
| **[x] Offline mode** (check-in in coda, sync alla riconnessione) | Alto (in teoria) | Alto | ✅ Fase A (backend, live in produzione) + Fase B (mobile, code-complete, Build 27 in corso su Codemagic) — Session 78-79, vedi `Offline Mode (~10h)` sotto e Task B6 in corso |
...
### Offline Mode (~10h) — ✅ Fase A+B implementate (Session 78-79)
...
- [ ] **Task B6** — ⏳ **IN CORSO (Session 81)**: test su iPhone reale quasi concluso, Build 33 pronta per il retest finale. Checklist `docs/offline-mode-test-checklist.md`: Sezioni 1-6 OK, Sezione 7 (device condiviso) bloccata da 2 bug ora fixati e verificati live (Pino `employee_id` + `assigned_sites`, migration 033), Sezione 8 bloccata da 1 bug ora fixato (`MyScheduleScreen` non refetchava su focus tab). 5 bug totali trovati/fixati in Session 81 (vedi sopra), più feature "richiedi sempre login dopo kill app". **Da riprendere**: lanciare Build 33 su Codemagic, retest completo Sezione 7 e 8, poi Sezione 4 tenendo conto del nuovo re-login richiesto dopo il kill
