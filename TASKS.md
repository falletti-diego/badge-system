...
**Last Updated:** 2026-07-18 (Session 76c: micro-manutenzione test/CI — flake token eliminato via globalSetup, marker bootstrap schema.sql, root cause del hang Jest trovata in pg-pool `min` e fixata, `--forceExit` rimosso; tutto pushato, CI+deploy verdi)  
...
- [ ] **`PUT /admin/sites/:id` (geofence) non ha un percorso `superadmin`** — scopa sempre su `req.user.client_id`, quindi un account `superadmin` (che non ha un tenant "proprio" significativo) non può aggiornare il geofencing di nessuna sede. Fail-closed (nessun rischio di sicurezza), ma un support/onboarding reale ci sbatterà contro. Preesistente, non introdotto da questa sessione.
- [ ] **Duplicazione del pattern `role==='superadmin' ? <valore chiamante> : req.user.client_id`** — ripetuto ~5 volte tra `sites.js`/`employees.js`/`clients.js`. Ogni istanza è stata giudicata accettabile isolatamente, ma vista tutta insieme meriterebbe un piccolo helper condiviso (es. `resolveTenantScope(req, data)`) per eliminare il rischio che una futura 5ª copia inverta la polarità.
- [ ] **Codici di errore inconsistenti per "non trovato o tenant altrui"** tra endpoint fratelli: `sites.js` DELETE → 400 `ValidationError`, `employees.js` DELETE → 404 `NotFoundError`, `clients.js` DELETE → 400 `ValidationError`. Nessuna fuga di informazioni in nessun caso, solo incoerenza per chi integra contro l'API.
- [ ] **`AdminSiteSchema`/`AdminEmployeeSchema` richiedono ancora `client_id` nel body** anche per un `admin` non-superadmin, il cui valore viene ora silenziosamente ignorato/sovrascritto — innocuo per la sicurezza, ma un wart di API/UX da risolvere se si costruirà mai un pannello self-service reale per i clienti.
...
- [x] **QA del funnel completo** (`/prova-demo` → dashboard → tour → banner → contatto → expired page) — ✅ **ESEGUITA Session 72 (2026-07-16)**, in locale (backend+Postgres+frontend Vite), funzionale via API + lettura codice (nessun tool di automazione browser disponibile in questo ambiente, quindi non è stata una QA visiva con screenshot). Verificato: `POST /demo/start` (200, JWT con `is_demo`/`demo_expires_at`), dashboard popolata con dati demo realistici (`/checkins`, `/presences/summary`, `/presences/trend`), `POST /demo/switch-role` (nuovo JWT con ruolo aggiornato), `POST /demo/contact` (200, richiesta salvata anche se l'invio email fallisce — fallimento gestito correttamente, non blocca la UX), `DemoTour.jsx`/`DemoBanner.jsx`/`DemoExpiredPage.jsx` (letti per intero, logica solida, nessun bug). **2 problemi reali trovati:**
  - **[ ] Durata trial incoerente**: il backend usa `DEMO_TENANT_EXPIRY_DAYS = '7 days'` (`backend/src/routes/demo.js:61`, confermato anche dalla risposta live di `/demo/start`: scadenza esattamente 7 giorni), ma il copy mostrato al prospect dichiara **14 giorni** in due punti: `TryDemoPage.jsx` ("max 14 giorni in tutto") e `DemoExpiredPage.jsx` ("dura fino a 14 giorni"). Un prospect legge 14 ma la demo scade a 7 — da decidere quale valore è quello voluto e allineare l'altro. Stima: 15 min una volta decisa la durata corretta.
  - **[ ] "Cosa vedrai" su `/prova-demo` mostra placeholder grigi**, non screenshot reali (`TryDemoPage.jsx` righe ~299-338, commento nel codice stesso lo conferma: "no real product screenshots are checked into the repo yet"). Per un prospect che vede la pagina per la prima volta, 3 box grigi con scritto "Dashboard — anteprima" comunicano il contrario di "prodotto pronto" — probabilmente il gap di percezione più immediato di tutta la pagina. Stima: 1h (screenshot reali + crop) una volta disponibili le immagini.

**🟡 Media — entro poche settimane dal lancio:**

- [ ] **CI senza servizio Postgres reale** — i test DB-dipendenti (inclusi quelli critici su race condition email duplicata, Task 3) girano solo in locale, mai in CI. Un futuro refactor potrebbe rompere silenziosamente questi path senza che CI se ne accorga. Stima: 2-4h.
...
- [ ] **Codice morto**: `frontend-web/src/lib/axiosInterceptor.js` — secondo interceptor axios registrato in `App.jsx`, controlla un campo `.code` mai popolato da nessun errore reale, completamente inerte (trovato durante Task 8). Confonde chi legge il codice, rischio di modifiche a un path che non esegue mai nulla. Stima: 0.5-1h.
