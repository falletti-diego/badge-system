...
- [ ] **RBAC cross-tenant `/api/admin/*`** — vedi finding HIGH sopra (stesso item, non duplicato). Stima: 2-3h decisione/design + 4-6h fix e test.
...
- [ ] **QA visiva su browser reale del funnel completo** (`/prova-demo` → dashboard → tour → banner → contatto → expired page) — mai eseguita in questo piano, solo test automatizzati frontend. I test coprono la logica, non l'impressione visiva/UX che un prospect vedrà per primo. Stima: 1-2h.
