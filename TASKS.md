...
**Last Updated:** 2026-07-30 (Session 88: ANDROID.2 completamente chiuso — root cause del blocco AVD trovata e risolta [PIN residuo da Session 83], verifica numerica su device reale completata, code review pulita)  
...
- [ ] **Infrastruttura reale non ancora provisionata** — ⚠️ **CONFERMATO con evidenza diretta AWS (Session 74), invio email FUNZIONANTE in produzione (Session 75)**: `aws ses list-identities` → nessuna identità verificata, `aws sesv2 get-account` → `ProductionAccessEnabled: false` (account SES in Sandbox). Fix applicati: `diego@dataxiom.it` verificato come mittente/destinatario (funziona solo verso quell'indirizzo, non verso prospect reali) + policy IAM `BadgeSESSendEmail` aggiunta al ruolo `badge-system-ec2-ecr-role` (mancava del tutto, causa del secondo blocco scoperto solo testando in produzione — vedi Session 75). **Resta da fare solo per prospect reali** (non più per l'uso interno, ora sbloccato): verifica dominio `dataxiom.it` via DNS (richiede accesso al pannello DNS) + richiesta di uscita dal Sandbox ad AWS, env var `MAX_ACTIVE_DEMOS` in produzione. ~~Regola scheduler per `cleanup-expired-demos.js`~~ ✅ fatta in Session 76 (cron su EC2 alle 3:30 UTC, scelto al posto di EventBridge). Stima residua: 3-5h (setup AWS, non codice).
