...
**Last Updated:** 2026-07-16 (Session 71: fix RBAC cross-tenant `/api/admin/*` completo su branch `worktree-admin-rbac-tenant-scoping`, 8/8 task del piano chiusi, in attesa di deploy produzione + decisione merge/PR)  
...
- [x] **Namespace `/api/admin/*` non scoped per tenant chiamante — divulgazione cross-tenant** (HIGH) — ✅ **FIXATO Session 71 (branch `worktree-admin-rbac-tenant-scoping`)**
  ...
  - **Ancora da fare (operativo, non bloccante per il merge di codice):** deploy in produzione delle Fasi 1-2, audit query per identificare gli account Dataxiom reali da promuovere a `superadmin`, verifica manuale post-deploy dell'`AdminPage` — checklist completa in Task 3 e Task 8 del piano.
