...
- [ ] **Infrastruttura reale non ancora provisionata**: verifica dominio/email mittente AWS SES, permesso IAM `ses:SendEmail` sul ruolo EC2, regola AWS EventBridge Scheduler per `cleanup-expired-demos.js`, env var `MAX_ACTIVE_DEMOS` in produzione. Il piano li tratta come "note operative fuori dal repo" — senza questi la demo funziona in locale ma non manda notifiche reali né si autopulisce in produzione. Stima: 3-5h (setup AWS, non codice).
