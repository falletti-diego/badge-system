...
- [ ] **Infrastruttura reale non ancora provisionata** — ⚠️ **CONFERMATO con evidenza diretta AWS (Session 74)**: `aws ses list-identities` → nessuna identità verificata, `aws sesv2 get-account` → `ProductionAccessEnabled: false` (account SES in Sandbox). Fix parziale applicato: `diego@dataxiom.it` verificato come mittente/destinatario (funziona solo verso quell'indirizzo, non verso prospect reali). **Resta da fare per prospect reali**: verifica dominio `dataxiom.it` via DNS (richiede accesso al pannello DNS) + richiesta di uscita dal Sandbox ad AWS, permesso IAM `ses:SendEmail` sul ruolo EC2 (da verificare), regola AWS EventBridge Scheduler per `cleanup-expired-demos.js`, env var `MAX_ACTIVE_DEMOS` in produzione. Stima residua: 3-5h (setup AWS, non codice).
